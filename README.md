...
*   `mcq_enabled` (BOOLEAN): Flag for multiple-choice questions.
*   `is_active` (BOOLEAN): Status of the template.
...
*   `id` (SERIAL, PK): Auto-increment ID.
*   `session_id` (UUID, FK): Associated session.
*   `alert_type` (VARCHAR): Type of anomaly (e.g., `no_face`, `tab_switch`).
*   `severity` (VARCHAR): Alert level (`warning`, `critical`).
