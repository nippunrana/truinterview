...
TruInterview is a robust, interactive, web-based technical assessment and screening platform. It leverages **Gemini 1.5 Flash** (via cURL integrations) and **TruGen.ai** to deliver real-time audio/video interviews, monitor candidate screen sharing, evaluate code submissions via AI vision, administer interactive multiple-choice tests, and generate comprehensive evaluation dashboards for both recruiters and candidates.
...
sequenceDiagram
    ...
    loop Interview Conversation
        Candidate->>Candidate: Shares Screen & Webcam
...
*   `current_status` (VARCHAR): State (`STARTED`, `IN_PROGRESS`, `MCQ_PROMPTING`, `MCQ_ACTIVE`, `COMPLETED`).
